...
do
  case "$i" in
    ...
    echo "\
  FROM alpine:3.9.6
  $(for j in Author GitHub; do echo "LABEL $j \"$($BASH_SOURCE $j)\""; done)
  ...
  RUN git clone $($BASH_SOURCE github) . && rm -fr .git"
    ...
    clean-none|clear-none) #removes all images with tag '<none>' as well as the corresponding containers
      for j in $(docker images | awk '/<none>/ {print $3}')
      do
        IDs=$(docker ps -a |awk '/'$j'/ {print $1}')
        [ -z "$IDs" ] || docker rm $IDs
        docker rmi $j
      done
    ;;
    ...
    clean-all|clear-all) #removes all relevant images and containers
      $BASH_SOURCE clean-exited clean-images clean-none
    ;;
